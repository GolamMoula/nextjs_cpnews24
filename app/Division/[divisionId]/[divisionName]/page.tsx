...
const page: FC<pageProps> = ({ }) => {
  ...
  const divisionId = segments[2];
  const divisionName = segments[3];
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const response = await axiosInstance.get(`news/division/${divisionId}`);
        const response_two = await axiosInstance.get(`all/district/${divisionId}`);
        setNewsData(response.data.News);
        setDistrictData(response_two.data.district);
      } catch (error) {
      ...
      <div>
        ...
        {districtData ? districtData.district.map(element => (
                  <li className="border rounded px-4 py-4 hover:bg-gray-200 text-2xl" key={element.id}>
                    ...
                    <Link href={`/District/${element.id}/${element.bn_name}/`}>
                    {element.bn_name}
                    </Link>
